...
//  wr.FSMC_AddressSetupTime = 15;
  wr.FSMC_AddressSetupTime = 1;	// revised on 03.05.2021
  wr.FSMC_AddressHoldTime = 0;
//  wr.FSMC_DataSetupTime = 0x3C;		// 0x3C=60
  wr.FSMC_DataSetupTime = 8;		// revised on 03.05.2021
...
//  w.FSMC_AddressSetupTime = 9;
  w.FSMC_AddressSetupTime = 1;
  w.FSMC_AddressHoldTime = 0;
  w.FSMC_DataSetupTime = 8;		// 0x3C=60
...
//>>>######################################################
  /* Set GRAM write direction and BGR = 1 */
    ...
    // bit 7 (page (row) addr. order) = 1  height ==> 0
		...
		//
  #else
		LCD_WriteData(0x48);	// bit 6, 3 = 1
		// bit 3 (BGR) = 1
		// bit 6 (column addr. order) = 1   rightward: 0 ==> width
    // bit 7 (page (row) addr. order) = 0  upward:  0 ==> height
		//
		//               column (x)
		//                       ^
		//                       |
...
//<<<######################################
		Ypos = (Ypos+height);
		if (Ypos > LCD_ROW_NUM) Ypos = 0;
		else Ypos =  LCD_ROW_NUM - Ypos;
	LCD_SetDisplayWindow(Xpos, Ypos, width, height);		// i.e., LCD_OpenWin(Xpos, y, Xpos+width-1, y+height-1)
