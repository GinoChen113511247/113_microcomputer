...
void LCD_FillTriangle(pPoint Points)
{
  ...
  uint16_t x, y;
	...
	//------------ 
	deltaY1 = (int16_t) yp[1]- yp[0];		// deltaY1 >= 0
  deltaX1 = (int16_t) xp[1]- xp[0];
		int_tmp = ((xp[2]-xp[0])*deltaY1) /(yp[2]-yp[0]);
		y_end2 = yp[2];
		yp[2] = yp[1];
		x_end2 = xp[2];
		xp[2] = (uint16_t)(xp[0] + int_tmp);
	
  deltaX2 = (int16_t) xp[2]- xp[0];
	...
	if (deltaY1 == 0){
		...
		for (i=0; i<=deltaY1; i++)
		{
				...
				LCD_DrawHLine((uint16_t) (x+x_tmp), y++, (uint16_t) ll+1);
		}	
	}
	//====== 2nd subtriangle 
		...
		y = y_end2;
	if (xp[1] < xp[2]){ 
		deltaY1 = (int16_t) y_end2- yp[1];		
		deltaX1 = (int16_t) xp[1]-x_end2;
		deltaX2 = (int16_t) xp[2]-x_end2;
	} else{
		deltaY1 = (int16_t) y_end2- yp[2];		
		deltaX1 = (int16_t) xp[2]-x_end2;
		deltaX2 = (int16_t) xp[1]-x_end2;
	}
